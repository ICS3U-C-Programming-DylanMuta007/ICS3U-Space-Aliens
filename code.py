...
# classifies the player
class Player(Ship):
  
  def __init__(self, x, y, health=100):
      super().__init__(x, y, health)
      self.ship_img = YELLOW_SPACE_SHIP  # Set the ship_img attribute to YELLOW_SPACE_SHIP
...
def collide(obj1, obj2): # helps to define collision
...
def main():
  ...
  level = 0
  lives = 3
  main_font = pygame.font.SysFont("Atari", 30) # font for the text
  lost_font = pygame.font.SysFont("Atari", 50) # font for the Lost text
...
def main_menu():
  title_font = pygame.font.SysFont("Atari", 30)
  run = True
  while run:
    WIN.blit(BG, (0,0))
    tittle_label = title_font.render("Press the mouse to begin...", True, 
    (0,255,0))
    pygame.display.update()
    for event in pygame.event.get():
      if event.type == pygame.QUIT:
